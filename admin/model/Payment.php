...
class Payment {
    ...
    public function getPayment($id)
    {
        ...
        if ($this->statement->execute()) {
            $payment = $this->statement->fetch(PDO::FETCH_ASSOC);
            return $payment;
        };
    }
    ...
    {
        $this->con = Database::connect();
        $sql = "INSERT INTO payment(method) VALUES (:payment)"; //string
    ...
    public function deletePayment($id)
    {
        $status = "deleted";
        $this->con = Database::connect();
        $sql = "update payment set status=:status where id=:id"; //string
        $this->statement = $this->con->prepare($sql); //prepare statement
        $this->statement->bindParam(":id", $id);
